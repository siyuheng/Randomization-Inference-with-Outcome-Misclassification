...
gle<-read.sas7bdat("/Users/heng6/Desktop/Measurement Error/PCPT/data/gleason_scores.sas7bdat")
...
severe<-rep(0, nrow(known))
known<-cbind(known, severe)

for (i in 1:nrow(known)){
  if (sum(as.numeric(gle$ALTPATID==known$ALTPATID[i] & gle$PRVCAGLT>=7 & gle$PRVCAGLT<=10))>0){
    known$severe[i]=1
  }
  if (sum(as.numeric(gle$ALTPATID==known$ALTPATID[i] & gle$PRVCAGLT>10))>0){
    known$severe[i]=-1
  }
  if (i%%1000==0){
    print(i)
...
table(data_no_missing$severe)
data_no_missing_with_or_without_gle<-data_no_missing
data_no_missing<-data_no_missing[data_no_missing$severe!=-1, ]
...
sum(strata_info_ava$N_ava) #9037 study subjects with gle
sum(strata_info_ava$N_t_ava) #4358 treated
sum(strata_info_ava$N_c_ava) #4679 control

sum(as.numeric(data_no_missing$severe==1 & data_no_missing$TRNO==1)) # 280
sum(as.numeric(data_no_missing$TRNO==1)) # 4358
sum(as.numeric(data_no_missing$severe==1 & data_no_missing$TRNO==2)) # 237
sum(as.numeric(data_no_missing$TRNO==2)) # 4679

sum(as.numeric(data_no_missing$severe==1 & data_no_missing$TRNO==1))/sum(as.numeric(data_no_missing$TRNO==1)) # 0.0642
sum(as.numeric(data_no_missing$severe==1 & data_no_missing$TRNO==2))/sum(as.numeric(data_no_missing$TRNO==2)) # 0.0507
...
for (i in 1:nrow(strata_info_ava)){
  strata_info_ava$Y_t[i]=sum(as.numeric(data_no_missing$ino==strata_info_ava$strata_no_ava[i] & data_no_missing$TRNO == 1 & data_no_missing$severe==1))
  strata_info_ava$Y_c[i]=sum(as.numeric(data_no_missing$ino==strata_info_ava$strata_no_ava[i] & data_no_missing$TRNO == 2 & data_no_missing$severe==1))
}
...
for (i in 1:nrow(strata_info_ava_final)){
  strata_info_ava_final$Y_t[i]=sum(as.numeric(data_no_missing_no_zeroNtNc$ino==strata_info_ava_final$strata_no_ava[i] & data_no_missing_no_zeroNtNc$TRNO == 1 & data_no_missing_no_zeroNtNc$severe==1))
  strata_info_ava_final$Y_c[i]=sum(as.numeric(data_no_missing_no_zeroNtNc$ino==strata_info_ava_final$strata_no_ava[i] & data_no_missing_no_zeroNtNc$TRNO == 2 & data_no_missing_no_zeroNtNc$severe==1))
}
...
sum(as.numeric(data_no_missing_no_zeroNtNc$severe == 1 & data_no_missing_no_zeroNtNc$TRNO==1))/sum(as.numeric(data_no_missing_no_zeroNtNc$TRNO==1)) #0.0644
sum(as.numeric(data_no_missing_no_zeroNtNc$severe == 1 & data_no_missing_no_zeroNtNc$TRNO==0))/sum(as.numeric(data_no_missing_no_zeroNtNc$TRNO==0)) #0.0506
...
Y<-data_no_missing_no_zeroNtNc$severe
sum(Z*Y)/sum(Z)
sum((1-Z)*Y)/sum(1-Z)
